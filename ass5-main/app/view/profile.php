    ...
    <div class="max-w-7xl mx-auto">
        <div class="flex justify-end mb-8 w-full">
            <nav class="bg-palette-dark border border-palette-mid px-2 py-2 rounded-full shadow-lg flex items-center gap-1 md:gap-2">
                ...
                <a href="index.php?action=profile" class="px-4 py-2 rounded-full text-sm font-semibold bg-palette-light text-white transition-all">ข้อมูลนักเรียน</a>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div class="lg:col-span-4 bg-palette-dark p-6 md:p-8 rounded-3xl shadow-xl border border-palette-mid">
                <h1 class="text-2xl font-bold text-white border-l-4 border-palette-light pl-4 mb-6">ข้อมูลนักเรียน</h1>

                <div class="flex flex-col gap-4">
                    <div class="bg-palette-bg/50 p-4 rounded-xl border border-palette-mid/50">
                        <p class="text-sm text-gray-400 mb-1">ชื่อ</p>
                        <p class="font-semibold text-lg text-white"><?= htmlspecialchars($user['first_name'] ?? 'ไม่พบข้อมูล') ?></p>
                    </div>
                    <div class="bg-palette-bg/50 p-4 rounded-xl border border-palette-mid/50">
                        <p class="text-sm text-gray-400 mb-1">นามสกุล</p>
                        <p class="font-semibold text-lg text-white"><?= htmlspecialchars($user['last_name'] ?? 'ไม่พบข้อมูล') ?></p>
                    </div>
                    <div class="bg-palette-bg/50 p-4 rounded-xl border border-palette-mid/50">
                        <p class="text-sm text-gray-400 mb-1">วันเกิด</p>
                        <p class="font-semibold text-lg text-white"><?= htmlspecialchars($user['birthdate'] ?? '0000-00-00') ?></p>
                    </div>
                    <div class="bg-palette-bg/50 p-4 rounded-xl border border-palette-mid/50">
                        <p class="text-sm text-gray-400 mb-1">เบอร์โทรศัพท์</p>
                        <p class="font-semibold text-lg text-white"><?= htmlspecialchars($user['phone'] ?? '0000000000') ?></p>
                    </div>
                </div>
            </div>

            <div class="lg:col-span-8 bg-palette-dark p-6 md:p-8 rounded-3xl shadow-xl border border-palette-mid">
                <h2 class="text-2xl font-bold text-white border-l-4 border-palette-light pl-4 mb-6">วิชาที่ลงทะเบียนเรียน</h2>

                <div class="overflow-x-auto rounded-xl border border-palette-mid">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-palette-mid text-white text-sm uppercase tracking-wider">
                        ...
                        <tbody class="divide-y divide-palette-mid bg-palette-dark">
                            <?php if (!empty($my_courses)): ?>
                                <?php foreach ($my_courses as $mc): ?>
                                    <tr class="hover:bg-palette-mid/50 transition-colors">
                                        <td class="p-4 font-medium text-gray-200 whitespace-nowrap"><?= htmlspecialchars($mc['course_code'] ?? '-') ?></td>
                                        <td class="p-4 text-gray-300"><?= htmlspecialchars($mc['course_name'] ?? '-') ?></td>
                                        <td class="p-4 text-gray-400 whitespace-nowrap"><?= htmlspecialchars($mc['instructor'] ?? '-') ?></td>
                                        <td class="p-4 text-gray-500 text-sm whitespace-nowrap"><?= date('d/m/Y H:i', strtotime($mc['created_at'])) ?></td>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="p-10 text-center text-gray-500 bg-palette-bg/50">
                                        <p class="text-lg">ยังไม่ได้ลงทะเบียนวิชาใดๆ</p>
                                        <a href="index.php?action=courses" class="text-palette-light hover:text-white hover:underline mt-2 inline-block transition-all">ไปลงทะเบียนกันเลย</a>
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
